...
class HuffmanTree(object):
    ...
    def __init__(self,char_weights):
        ...
        while len(self.Leav)!=1:
            ...
            c=Node(value=(self.Leav[-1]._value + self.Leav[-2]._value + self.Leav[-3]._value))
            c._left=self.Leav.pop(-1)
            c._right=self.Leav.pop(-1)
            c._con = self.Leav.pop(-1)
            self.Leav.append(c)
    ...
    def pre(self,tree,length):
        ...
        self.pre(node._con,length+1)
     #生成哈夫曼编码
    def get_code(self):
        self.pre(self.root,0)
if __name__=='__main__':
    #输入的是字符及其频数
    char_weights=[('a',0.2),('b',0.18),('c',0.15),('d',0.12),('f',0.09),('g',0.08),('h',0.06),('i',0.04),('j',0.04),('k',0.02)]
